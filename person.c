...
void addPerson()
{
	// v tejto metode pouzivame kniznicne volania
	if (user_cnt) closeDB();
...
int openDB(void)
{
	// tu su pouzite systemove volania
	
	// ak chceme pisat na standardny chybory vystup, musime to dat
	// pomocou funkcie fputs.......stderr...
	struct stat finfo;
	...
	// zisti velkost suboru naplni finfo datamy z inodu
	fstat(db_fd, &finfo);
	if (finfo.st_size > 0) {
...
void removePerson()
{
	...
	if ((por) && (por<=user_cnt)) {
		if (por == user_cnt) {
			// pri poslednom len orezem databazu
			closeDB();
			// "oreze" velkost suboru
			truncate(DB_FILE, sizeof(person)*(por-1));
		} else {
			// pri inom vymenim posledneho s nim a orezem databazu
